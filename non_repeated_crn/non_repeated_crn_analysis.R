...
mod <- cmdstan_model("non_repeated_crn_model.stan"
...
chain_1 <- fit@sim[["samples"]][[1]][,c(1:6)]
chain_2 <- fit@sim[["samples"]][[2]][,c(1:6)]
chain_3 <- fit@sim[["samples"]][[3]][,c(1:6)]
...
x2.sim <- seq(min(stan.df$climate), max(stan.df$climate), by = 0.02)
...
df.posteriors <- data_frame(Submodel = c(rep("Growth", 3000*2), rep("Fecundity", 3000*2))
                            ...
                            , Posterior = c(dat_plot$mu_growth, dat_plot$beta_g
