#### This R script contains postprocessing functions for CRN models and was taken from Martin, J. S. (2023). Covariance reaction norms: A flexible approach to estimating continuous environmental effects on quantitative genetic and phenotypic (co)variances. EcoEvoRxiv. doi: https://doi.org/10.32942/X2D89H 
#### It can also be found on Jordan Martin's GitHub at https://github.com/Jordan-Scott-Martin/covariance-reaction-norms 
...
cor_f = function(x, traits, cpc){
  ...
  cor_pl = cbind(cor_pl, x[rep(seq_len(nrow(x)), each = length(cpc)), ]) 
