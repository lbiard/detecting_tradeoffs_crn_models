...
data {
  ...
  array[N] int<lower=0> idc_g; //index of repro event - growth
  array[N] int<lower=0> id_g_lm; //index of observations - growth
  ...
  array[M] int<lower=0> idc_f; //index of repro event - fecundity
  array[M] int<lower=0> id_f_lm; //index of observations - fecundity
...
parameters { 
  //fixed effects
  vector[P_g] B_mq_g; //RN of means
  vector[P_f] B_mq_f; //RN of means
...
model {
  ...
  vector[N] mu_g = mu_growth[id_g_lm];
  vector[M] mu_f = mu_fecundity[id_f_lm];
